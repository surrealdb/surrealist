...
export function NetworkEgressChart({
	instance,
	duration,
	nodeFilter,
	calculateNodes,
}: ObserverChartParentProps) {
	const { data: metrics, isPending } = useCloudMetricsQuery(instance.id, "egress", duration);
	...
	return (
		<ObserverChart
			...
			information="The outgoing network traffic measured in kilobytes per second"
			duration={duration}
			values={values}
			series={series}
			tooltip={tooltip}
