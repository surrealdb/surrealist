...
export function readQuery(tab: QueryTab): Result<string> {
	return tab.queryType === "config" ? CONFIG_STRATEGY.read(tab) : FILE_STRATEGY.read(tab);
}
...
export function writeQuery(tab: QueryTab, query: string) {
	tab.queryType === "config"
		? CONFIG_STRATEGY.write(tab, query)
		: FILE_STRATEGY.write(tab, query);
}
