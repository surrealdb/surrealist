...
export function TicketPartAuthorDetails({ user, updated_at, children }: TicketPartAuthorDetailsProps) {
    return (
        <Group
            gap="xl"
            wrap="nowrap"
        ...
        >
            <Avatar
                ...
                src={user?.avatar ?? ((!user || user?.type === "admin" || user.type === "bot") ? surrealImg : undefined)}
            ...
            <Stack>
                <Stack gap={0}>
                    <Group gap="xs">
                        <Text c="bright" fz="lg" fw={700}>{user?.name ?? "SurrealDB Team"}</Text>
                        {user?.type === "admin" && (
                            <Badge
                                size="sm"
                                variant="light"
                    ...
                    <Text fz="sm" c="slate.3">{formatRelativeDate(updated_at * 1000)}</Text>
                </Stack>
                {children}
            </Stack>
        </Group>
