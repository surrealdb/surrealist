...
function ObjectCell(props: { value: any }) {
	return (
		<div>
			<HoverCard width={280} shadow="md" withArrow position="bottom-start">
				<HoverCard.Target>
					<Text span ff="JetBrains Mono" style={{ cursor: "help" }}>
...
function GeographyPointCell({ value }: { value: GeometryPoint }) {
	...
	try {
		...
		return (
			<GeographyLink value={value} text={converted.toCoordinateFormat("DMS")} />
		);
	} catch (err) {
		console.error(err);
		return (
