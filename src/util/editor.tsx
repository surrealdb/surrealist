...
export function initializeEditor(monaco: Monaco) {
	...
	monaco.languages.setMonarchTokensProvider('surrealql', {
		ignoreCase: true,
		keywords: [
			'AFTER', 'ANALYZE', 'ANALYZER', 'ASC', 'AS', 'ASSERT', 'BEFORE', 'BEGIN', 'BM25', 'CANCEL', 'COLUMNS', 'COMMIT', 'CONTENT',
			'CREATE', 'DATABASE', 'DB', 'DEFINE', 'DELETE', 'DESC', 'DESCRIBE', 'DIFF', 'DROP', 'ELSE', 'END', 'ES256', 'ES384', 'ES512',
			'EVENT', 'EXPLAIN', 'FETCH', 'FIELD', 'FILTERS', 'FOR', 'FROM', 'FUNCTION', 'GROUP', 'BY', 'HIGHLIGHTS', 'HS256', 'HS384',
			'HS512', 'IF', 'INDEX', 'INFO', 'INSERT', 'IGNORE', 'INTO', 'KILL', 'LET', 'LIMIT', 'LIVE', 'LOGIN', 'MERGE', 'NAMESPACE',
			'NS', 'ON', 'DUPLICATE', 'KEY', 'UPDATE', 'ORDER', 'PASSHASH', 'PASSWORD', 'PERMISSIONS', 'PS256', 'PS384', 'PS512', 'RELATE',
			'REMOVE', 'REPLACE', 'RETURN', 'RS256', 'RS384', 'RS512', 'SCHEMAFULL', 'SCHEMALESS', 'SCOPE', 'SEARCH', 'SELECT', 'SESSION',
			'SET', 'SIGNIN', 'SIGNUP', 'SLEEP', 'SPLIT', 'ON', 'START', 'AT', 'TABLE', 'TOKENIZERS', 'THEN', 'TIMEOUT', 'TOKEN', 'TYPE',
			'UNIQUE', 'UPDATE', 'USE', 'VALUE', 'VALUES', 'VERSION', 'WHEN', 'WHERE', 'AND', 'OR', 'IS', 'NOT', 'CONTAINSALL', 'CONTAINSANY',
			'CONTAINSNONE', 'CONTAINSSOME', 'CONTAINSNOT', 'CONTAINS', 'ALLINSIDE', 'ANYINSIDE', 'NONEINSIDE', 'SOMEINSIDE', 'NOTINSIDE',
			'NOT IN', 'INSIDE', 'OUTSIDE', 'INTERSECTS'
		],
		tokenizer: {
			root: [
				[/(count|(\w+::)+\w+)(?=\()/, 'function'],
